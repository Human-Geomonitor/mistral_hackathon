"""Runfola, Daniel, Community Contributors, and [v4.0: Lindsey Rogers, Joshua Habib, Sidonie Horn, Sean Murphy,
Dorian Miller, Hadley Day, Lydia Troup, Dominic Fornatora, Natalie Spage, Kristina Pupkiewicz, Michael Roth,
Carolina Rivera, Charlie Altman, Isabel Schruer, Tara McLaughlin, Russ Biddle, Renee Ritchey, Emily Topness,
James Turner, Sam Updike, Helena Buckman, Neel Simpson, Jason Lin], [v2.0: Austin Anderson, Heather Baier,
Matt Crittenden, Elizabeth Dowker, Sydney Fuhrig, Seth Goodman, Grace Grimsley, Rachel Layko, Graham Melville,
Maddy Mulder, Rachel Oberman, Joshua Panganiban, Andrew Peck, Leigh Seitz, Sylvia Shea, Hannah Slevin,
Rebecca Yougerman, Lauren Hobbs]. "geoBoundaries: A global database of political administrative boundaries." Plos one
15, no. 4 (2020): e0231866. """
...
import pygeoboundaries.countries_iso_dict as countries_iso_dict
import pygeoboundaries.iso_codes as iso_codes
import requests
...
def _is_valid_adm(iso3, adm: str) -> bool:
    html = _session.get(
        "https://www.geoboundaries.org/api/current/gbOpen/{}/".format(iso3),
        verify=False,
    ).text
...
def _validate_adm(adm: str | int) -> str:
    if type(adm).__name__ == "int" or len(str(adm)) == 1:
        adm = "ADM" + str(adm)
    if (
        str.upper(adm) in ["ADM{}".format(str(i)) for i in range(6)]
        or str.upper(adm) == "ALL"
    ):
...
def _generate_url(territory: str, adm: str | int) -> str:
    ...
    if not _is_valid_adm(iso3, adm):
        print(
            "KeyError : ADM level '{}' doesn't exist for country '{}' ({})".format(
                adm, territory, iso3
            )
        )
        raise KeyError
...
def get_metadata(territory: str, adm: str | int) -> dict:
    ...
    """
    return _session.get(
        _generate_url(territory, adm), verify=False
    ).json()  # TO DO get rid of verify arg
...
def _get_data(territory: str, adm: str, simplified: bool) -> dict:
    ...
    try:
        ...
        print(
            "Error while requesting geoboudaries API\n URL : {}\n".format(
                _generate_url(territory, adm)
            )
        )
        raise
    return _session.get(json_uri, verify=False).text
...
def get_adm(territories: str | List[str], adm: str | int, simplified=True) -> dict:
    ...
    Allowed values for <territories> argument :
        ...
        - ISO 3166-1 (alpha2) : AFG, QAT, YEM, etc. - ISO 3166-1 (alpha3) : AF, QA, YE, etc. - Country name (i.e ADM0
        territories) in any of the following languages : Arabic, Armenian, Basque, Bulgarian, Chinese (simplified),
        Chinese (traditional), Czech, Danish, Dutch, English, Esperanto, Estonian, Finnish, French, German, Greek,
        Hungarian, Italian, Japanese, Korean, Lithuanian, Norwegian, Polish, Portuguese, Romanian, Russian, Slovak,
        Spanish, Swedish, Thai, Ukrainian For more information, check out
        https://stefangabos.github.io/world_countries/ (the data source for ISO codes and countries' names)
    ...
    for i in territories:
        geojsons += geojson.loads(_get_data(i, adm, simplified))["features"]
